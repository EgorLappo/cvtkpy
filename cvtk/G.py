...
def convergence_corr_from_freqs(freqs, depths=None, diploids=None, **kwargs):
    R, Tp1, L = freqs.shape
    T = Tp1 - 1
    covs = temporal_replicate_cov(
        freqs, depths=depths, diploids=diploids, **kwargs)
...
def convergence_corr_denominator(stacked_temporal_covs):
    ...
    # mask na's
    vars = np.ma.array(vars, mask=np.isnan(vars))
    ...
    sdmat = np.sqrt(np.nanmean(varmat[:, tr, tc, :, :], axis=1))
    return sdmat
